...
export default class Bird {
    ...
    jump() {
        ...
        TweenMax.to(this.mesh.position, totalSpeed / 2, {
            y: -100, ease: Power4.easeIn, delay: totalSpeed / 2, onComplete: function () {
    ...
    stand() {
        ...
        TweenMax.to(this.mesh.rotation, sp, { x: 0, ease: ease, onComplete: function () {
            _this.status = "standing";
        } });
    }
    ...
    reset() {
        switch (this.currentScene) {
            ...
            case (3):
                this.lookAway(false);
                if (this.status === "chilling") { this.stand(); }
                break;
            case (4):
                clearInterval(this.keepJumping);
                break;
